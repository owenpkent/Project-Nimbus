...
class RudderSettingsDialog:
    """
    Dialog window for configuring rudder sensitivity, deadzone, and extremity deadzone with graph.
    """
    
    def __init__(self, config: ControllerConfig, parent_surface: pygame.Surface):
        """
        Initialize the rudder settings dialog.
    ...
    def load_current_settings(self) -> None:
        """Load current rudder settings from configuration."""
        self.sensitivity = self.config.get("rudder_settings.sensitivity", 50.0)
        self.deadzone = self.config.get("rudder_settings.deadzone", 10.0)
        self.extremity_deadzone = self.config.get("rudder_settings.extremity_deadzone", 5.0)
    ...
    def save_settings(self) -> None:
        ...
        self.config.set("rudder_settings.sensitivity", self.sensitivity)
        self.config.set("rudder_settings.deadzone", self.deadzone)
        self.config.set("rudder_settings.extremity_deadzone", self.extremity_deadzone)
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        # Draw title
        title_text = self.title_font.render("Rudder Settings", True, self.text_color)
    ...
    def _draw_graph(self, surface: pygame.Surface) -> None:
        """Draw the rudder sensitivity curve graph."""
