...
**Spectator+ — AI-assisted execution.** A trained agent translates high-level intent (issued by voice, dwell, or switch) into precise sequences of axis and button events through the existing virtual driver bridge. The user remains the cognitive and tactical authority; the AI executes the motor sequence. The first step is in the tree without a model: scripted primitives (turn by an angle, walk for a distance, press a button) executed by the bridge from a per-game calibration that a game test harness measures from the game's own console, and verified in a real game to within a few degrees (`docs/vision/GAME_TEST_HARNESS.md`, section 4.7).
