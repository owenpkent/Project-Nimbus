...
#### Measured thresholds, and the default that cannot serve all four (2026-09-07, extended 2026-09-08)

The harness has now put four games' right-stick thresholds on the same scale, and the default floor of **0.289** (anti-deadzone 26.5 plus buffer) sits in a different place relative to each:

| Game | Threshold | Where 0.289 lands | Through Nimbus, 1 px drag |
|---|---|---|---|
| Elden Ring | at or below 0.20 | comfortably above | moves (frame verdict) |
| Left 4 Dead 2 | 0.26 to 0.28 | just above, by design | 2.4 degrees a second |
| Half-Life 2 | 0.30 to 0.40 | **below it** | **nothing at all** |
| PowerWash Simulator | below 0.26 (0.20 marginal) | comfortably above | moves (frame verdict, 10,516 samples) |

Half-Life 2 is the case the default gets wrong. The bridge does exactly what it should, sending its 0.289 floor for a one-pixel drag, and the camera does not move, because this game does not start turning until somewhere between 0.30 and 0.40. So the smallest movement a Nimbus user can make is invisible there, which is the same on-off complaint section 2.2 describes, arrived at from the other side: not the game eating the first quarter of our range, but our floor sitting under the game's own threshold. Left 4 Dead 2's 0.28, Elden Ring's 0.20 and PowerWash Simulator's sub-0.26 bracket it on the other side, so no single constant is right for all four, and Half-Life 2 needs its Anti-DZ raised to about 40 where Elden Ring and PowerWash want it lowered. Three of the four sit below the default and one above it, which is the shape of the problem: the outlier is not rare enough to ignore and not common enough to design the default around.
