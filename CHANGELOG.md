...
### Documentation
- **Pad Bus Fork Plan** (`docs/vision/PAD_BUS_FORK_PLAN.md`): What it would take to fork and modernize ViGEmBus into a virtual gamepad bus driver we own, checked against a fresh clone of the upstream source on 2026-09-09 rather than from memory. Upstream was archived on 2023-11-02 with 1.22.0 as its final release, the announced successor has never appeared, and none of its 403 forks is maintained (the most active is two commits ahead on an audio experiment its author marked "bad stability"), so anyone wanting a maintained copy has to become its maintainer. The driver is 6,484 lines of C++, of which DualShock 4 is 1,547 that Nimbus needs none of, and the real asset is roughly 1,500 lines of USB device-stack emulation: the child PDO answers `IOCTL_INTERNAL_USB_SUBMIT_URB` with hand-written descriptors so that Microsoft's inbox `xusb22.sys` binds to it. That is also why the plan rejects the alternatives the VDroid brainstorm preferred, and why that document now carries a correction: `XInput1_4.dll` only sees devices bound to `xusb22.sys`, which binds to the USB compatible ID `USB\MS_COMP_XUSB10`, so VHF, UMDF HID and `vhidmini2` cannot do XInput at any level of effort. A second correction: the 8 axis ceiling blamed on vJoy is `DIJOYSTATE2`'s shape, so a wider HID device gives games no more axes and the honest case for one is dropping the vJoy dependency, not escaping it. Two findings decide the order of work. Upstream's INF carries a `CoInstallers32` / `WdfCoInstaller` section, which fails `InfVerif /k` and is rejected by attestation signing outright, so it cannot be signed as written; and its build infrastructure is unusable by anyone else (NUKE, an AppVeyor account that authenticates as its author, Advanced Installer, and a 647-line patch whose only purpose is pinning Microsoft's DMF to WDK 19041, obsolete on retarget). So the plan leads with phase 0, a pure-`ctypes` client speaking the ten-IOCTL protocol directly, which retires `vgamepad`, ends the 1.17.333.0 versus 1.22.0 split on dev machines, deletes the CI step that filters vgamepad out of `requirements.txt`, and carries no kernel risk. The fork itself is gated, and the gate is not engineering: ViGEmBus is tolerated by EAC and BattlEye largely through years of ubiquity via DS4Windows, and a renamed fork inherits none of that reputation, so a technically better driver could leave users unable to play. Also covers the new repo (`nimbus-padbus`, separate from this one, BSD-3-Clause with the upstream copyright retained, named to avoid both the ViGEM GmbH trademark and BSD clause 3), the rename and GUID inventory that lets it coexist with an existing ViGEmBus install, a kernel failsafe modelled on the mouse filter's heartbeat so a hung Nimbus cannot leave a stick pinned, and a test plan that reuses the game harness to prove behaviour compatibility in a real game rather than asserting it. `docs/architecture/VDROID_DRIVER_BRAINSTORM.md` and `VDROID_OPTION_E_IMPLEMENTATION.md`, `driver/SIGNING.md` section 7 and `docs/setup/PACKAGING.md` updated to match; the last of those had claimed ViGEmBus "is the path that has a future", which was already untrue when it was written.
