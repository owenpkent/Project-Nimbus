...
### Documentation
- **Linux research recorded** — `docs/vision/HOST_MODE_ISOLATION.md` gains a measured-results table for the Linux host question (uinput pad recognised by SDL and Steam Input, X11 no-focus flag verified, EVIOCGRAB isolation confirmed with `tests/probe_evdev_grab.py`, Wayland untested), `docs/vision/LINUX_PROBE_PLAN.md` records per-criterion results, and `docs/WHITEPAPER.md` describes the Linux runtime, the uinput back end (§ 6.3), and the X11 Game Focus Mode equivalent. `docs/setup/LINUX.md` and `docs/GAME_COMPATIBILITY.md` document Game Mode on Linux. Mouse Isolation was then confirmed in real Proton games: Carrier Command 2 (free-look camera blind to a grabbed mouse, uinput pad recognised as an Xbox 360 controller) and Elden Ring under Easy Anti-Cheat (online session accepted, Xbox glyphs, pad drives the camera, raw-input mouse-look blind while isolated), closing every criterion in the Linux probe plan. `tests/probe_game_mouselook.py` reproduces the in-game measurement against any running game window.
...
### Added
- **Always on Top** (View menu) — Pins the Nimbus window above other windows via Qt's `WindowStaysOnTopHint` (`WS_EX_TOPMOST` on Windows, `_NET_WM_STATE_ABOVE` on X11), which is what makes the panel usable beside a **fullscreen** game; without it the game covers the UI, and on Linux that also hides the Mouse Isolation software cursor and the Game Mode button needed to stop. The choice is saved (`ui.always_on_top`) and restored on the next launch. Full Game Mode pins the window for the session and unpins it on stop, leaving a user-set pin alone. Reported unavailable on Wayland, which has no client-settable "above" state. `tests/probe_always_on_top.py` measures it: on GNOME Shell / X11 a pinned panel kept 100% of its pixels while a fullscreen window held focus, against 0% unpinned.
- **Linux support** (`src/uinput_interface.py`) — The app now runs on Linux. Two kernel `uinput` virtual devices stand in for the Windows drivers with identical method names: `UInputXboxInterface` (an `Microsoft X-Box 360 pad` with the same VID/PID the `xpad` driver reports, so Steam/SDL apply their built-in mapping) replaces ViGEm, and `UInputJoystickInterface` (8 axes, 56 buttons) replaces vJoy. Pure Python via `ioctl`/`struct`, no compiled dependency. The bridge picks the platform back end through `_create_xbox_interface()` / `_create_joystick_interface()` and keeps only the active device alive on Linux. Adds `run.sh`, `tests/test_uinput.py` (round-trip diagnostic), `build_tools/linux/60-nimbus-uinput.rules`, and `docs/setup/LINUX.md`. `requirements.txt` now marks `pyvjoy` and `vgamepad` as Windows-only; the driver modules tolerate any import error instead of only `ImportError`.
- **Game Mode on Linux** (`src/controller_pulse.py`) — The controller-mode keep-alive pulse (initial 0.5 burst + A press, then a 30 Hz sub-deadzone left-stick oscillation that restores the user's real stick values every tick) is now driver-agnostic and drives the uinput Xbox pad. Off Windows the bridge routes `startControllerMode` / `startFullGameMode` through it, and Game Focus Mode is implemented with Qt's `WindowDoesNotAcceptFocus` flag on X11 (verified: the window keeps receiving pointer input while keyboard focus stays with the game). The ribbon **Game Mode** button therefore works on Linux without the Windows window picker. Borderless conversion, ClipCursor release, the mouse hook, and the `Ctrl+Alt+F12` hotkey remain Windows-only.
- **Mouse Isolation (Linux)** (`src/mouse_isolation.py`) — View > Isolate Mouse takes an exclusive `EVIOCGRAB` on every physical pointer device so the X server, Wayland, Steam, and any game stop receiving the mouse; the bridge reads the deltas, keeps a software cursor (drawn in `Main.qml`), and delivers synthetic `QMouseEvent`s to its window so all widgets keep working. Keys from keyboard-and-touchpad combo devices are re-emitted through a uinput pass-through keyboard; `Ctrl+Alt+F12` releases from any keyboard; the kernel drops the grab on exit. Game Mode enables it by default (`controller.game_mode_isolate_mouse`). Verified end to end with a grabbed virtual device clicking the Game Mode button while the desktop pointer stayed still.
