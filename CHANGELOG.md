...
### Added
- **Installer driver probe** (`tests/probe_installer_drivers_windows.ps1`): Eleven numbered checks over the installer's driver bootstrap, unattended after the elevation prompt. Records the baseline, optionally removes vJoy and ViGEmBus (`-Fresh`) so the install path is exercised rather than skipped, runs the installer with `/S`, then verifies against the machine rather than the exit code: both drivers back, vJoy device 1 reporting 8 axes and 128 buttons through `vJoyInterface.dll`, no leftover `drivers\` folder (which only appears when an install failed), the app starting and showing a window, and finally the app uninstalling while both drivers survive.
