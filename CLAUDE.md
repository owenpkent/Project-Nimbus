...
- Real-game tests: `tests/probe_game_harness_windows.py --game left4dead2 --actuator pad` (calibrates the game through its console, about four minutes, unattended, launches and quits the game itself) and `--actuator nimbus` (the real app end to end). Recipes are `tests/games/*.json`; the machinery is `tests/game_harness.py`; the plan and the results log are `docs/vision/GAME_TEST_HARNESS.md`. Run one at a time so each session's pad is player one.
